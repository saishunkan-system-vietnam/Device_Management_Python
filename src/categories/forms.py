...
class CategoriesForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if len(category_name) > 100:
            self.errors['category_name'] = self.error_class(
                ['Không được nhập quá 100 ký tự.'])
        if not category_name:
            self.errors['category_name'] = self.error_class(
                ['Category name là bắt buộc.'])
        id_parent = int(self.cleaned_data.get('id_parent'))
        if (not id_parent or id_parent == None) & id_parent != 0:
            self.errors['id_parent'] = self.error_class(
                ['Category parent là bắt buộc.'])
